...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`async-io`](https://crates.io/crates/async-io).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `async-io` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Async wrappers around blocking I/O sources
...
Original work copyrighted by the upstream `async-io` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
